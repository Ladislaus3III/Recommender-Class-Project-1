...
class DataPreprocessingToolkit(object):
    ...
    def filter_out_long_stays(df):
        ...
        :rtype: pd.DataFrame
        """
        condition = df['length_of_stay'] <= 21
    ...
    def filter_out_low_prices(df):
        ...
        :rtype: pd.DataFrame
        """
        condition = df['accommodation_price'] > 50.0
    ...
    def add_weekend_stay(df):
        ...
        :rtype: pd.DataFrame
        """
        DF = df['date_from'].dt.weekday
        DT = df['date_to'].dt.weekday
        d = (df["date_to"] - df["date_from"]).dt.days
        df['weekend_stay'] = (((d >= 6) | (DF >= 4) & (DF != 6)) | (DT >= 5) | ((DT < DF) & (DF != 6)))
        
        # Without this assertion won't pass
